!function(root, factory) {
        ...
        function factory(fetch, Header, FormData) {
            function fetcher(actionUrl) {
                ...
                var fetchOptions = {
                    ...
                    headers: headers,
                    body: body
                }, fetchPromise = new Promise(function(accept, reject) {
                    fetch(actionUrl, fetchOptions).then(function(response) {
                        response.ok !== !1 ? convertResponseContent(response, acceptMimeType).then(function(data) {
                            accept({
                                data: data,
                                response: response
                            });
                ...
                return options.minDelay ? Promise.all([ fetchPromise, Promise.delay(options.minDelay) ]).then(function(values) {
            ...
            function createBody(data, format) {
                ...
                if ("form" === format) {
                    for (var formDataNameValue = flattenFormData(data), formData = new FormData(), i = 0; i < formDataNameValue.length; ++i) {
                        var name = formDataNameValue[i][0], value = formDataNameValue[i][1];
    ...
    function(module, exports) {
        ...
        function flattenArray(data) {
            for (var flattendData = arguments.length > 1 && void 0 !== arguments[1] ? arguments[1] : [], prefix = arguments.length > 2 && void 0 !== arguments[2] ? arguments[2] : [], i = 0; i < data.length; ++i) {
                var keyPath = prefix.concat([ "" ]), value = data[i];
                isIterable(value) ? flattenArray(value, flattendData, keyPath) : isObject(value) ? flattenObject(value, flattendData, keyPath) : flattendData.push([ makeKeyName(keyPath), "object" === ("undefined" == typeof value ? "undefined" : _typeof(value)) ? value.valueOf() : value ]);
            }
        }
        ...
        function makeKeyName(keyPath) {
            ...
            return keyPath.length ? initialKey + "[" + keyPath.join("][") + "]" : initialKey;
        }
